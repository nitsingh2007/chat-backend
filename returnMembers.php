...
where a.lastlogindate >= trunc(sysdate)
and a.lastlogindate < trunc(sysdate)+1 and a.lastloggedin is NOT NULL and a.lastloggedout is NULL order by LOWER(a.username) ASC";
   $stmt=oci_parse($conn,$query);
...
continue;
  ...
  /* $query='select club from usersofchat where username=:U';
   $stmt=oci_parse($conn, $query);
   oci_bind_by_name($stmt, ':U', $Username);
   if (!oci_execute($stmt)) {
  $e = oci_error($stmt);
 ...
 $Group='';
    ...
    if($Group==null || $Group=='')
  {
     echo json_encode([
    "status" => "Failure",
    "message" => "No Chat Participant Exists"
  ]);
  exit;
  }
  ...
  //for($i=0; $i<sizeof($ArrayOfUsernames);$i++)
   // {
     // $ProfilePic='';
      $query='select username,profilepic from usersofchat where username in :A';
      $stmt=oci_parse($conn, $query);
      oci_bind_by_name($stmt, ':A', $ArrayOfUsernames);
      if(!oci_execute($stmt))
        {
      $e = oci_error($stmt);    
      echo json_encode([
  ...
  exit;
    ...
    {
       // echo "Row is: ".$row;
      $response[]=['username'=>$row['USERNAME'], 'profilepic'=>$row['PROFILEPIC']];
