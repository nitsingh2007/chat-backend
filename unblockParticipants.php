...
echo json_encode([
        "status" => "FAILURE",
        "message" => " WRONG  CHOICE-PARTICIPANT WAS NOT BLOCKED"
    ]);
    exit;
...
//$ArrayOfUsernames = array_map('strtoupper', $ArrayOfUsernames);
 if(!in_array($Participant, $ArrayOfUsernames))
  ...
  echo json_encode( ["status" => "Failure",
        "message" => "Participant Was Not Already Blocked"
    ]);
   exit;
  }
 //$Group=$Group.'#~#'.$Participant;
 $index=array_search($Participant,$ArrayOfUsernames);


  

 unset($ArrayOfUsernames[$index]);
 $ArrayOfUsernames = array_values($ArrayOfUsernames);
 $Group=implode('#~#', $ArrayOfUsernames);
 $query = "update usersofchat set blocked=:G where username=:S";
  ...
  echo json_encode( ["status" => "Success",
        "message" => "Chat Participant Unblocked"
    ]);
