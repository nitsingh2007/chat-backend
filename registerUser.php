...
if($Username==null || $Password==null || $Email==null || $Otp==null)
{
...
$query1="select otp from tempotptable where username=:U and email=:E
...
try{
$query1="insert into usersofchat(email, username, password) values(:E,:U,:P)";
$stmt=oci_parse($conn, $query1);
oci_bind_by_name($stmt, ":E", $Email);
oci_bind_by_name($stmt, ":U", $Username);
...
if(!$result)
    {
     ...
     if($error['code']==1)   
        ...
        $response=[
            'status'=>'Failure',
            'message'=>'The Email/Username already in use.Please register with different Email/Username or use Forgot Username Option',
            ];
  ...
  echo(json_encode($response));  
    exit;
        ...
        $response=[
            'status'=>'Success',
            'message'=>'The Username has been succesfully registered.',
            ];
            $deleteQuery = "delete from tempotptable where username=:U and email=:E";
